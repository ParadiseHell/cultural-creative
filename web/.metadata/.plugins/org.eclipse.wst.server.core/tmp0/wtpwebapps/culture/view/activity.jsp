		...
		<div class="row">
			...
			<div class="col-md-9 user-right">
				...
				<nav style="text-align:center">
					...
					%>
				  <ul class="pagination bottom-nav" >
				  	<%
			  		if(currentPage == 1){%>
				  	...
				  	for(int i = currentPage - 1; i >= currentPage - 2; i--){
				  		if(i >= 1){%>
				  			<li><a href="<%=request.getContextPath()%><%=ParamUtils.SERVLET_ACTIVITY%>?<%=ParamUtils.TAB%>=<%=mTab%>&<%=ParamUtils.PAGE%>=<%=i%>"><%=i%></a></li>
				  		<%}%>
				  	<%} %>
				    <li class="active"><a href="<%=request.getContextPath()%><%=ParamUtils.SERVLET_ACTIVITY%>?<%=ParamUtils.TAB%>=<%=mTab%>&<%=ParamUtils.PAGE%>=<%=currentPage%>"><%=currentPage %></a></li>
				    <%
				  	for(int i = currentPage + 1; i <= currentPage + 2; i++){
					  	if(i <= lastPage){%>
					  		<li><a href="<%=request.getContextPath()%><%=ParamUtils.SERVLET_ACTIVITY%>?<%=ParamUtils.TAB%>=<%=mTab%>&<%=ParamUtils.PAGE%>=<%=i%>"><%=i%></a></li>
			  		...
			  		<%}else{%>
				  		 <li><a href="<%=request.getContextPath()%><%=ParamUtils.SERVLET_ACTIVITY%>?<%=ParamUtils.TAB%>=<%=mTab%>&<%=ParamUtils.PAGE%>=<%=lastPage%>">尾页</a></li>
				  	<%}%>
				  </ul>
				</nav>	    
			</div>
