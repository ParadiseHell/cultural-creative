...
public class CompanyImpl extends BaseImpl implements CompanyUserDao {
	...
	private final static String MODIFY = "call modifyCompanyUser(?,?,?,?,?,?,?,?,?,?)";
	...
	@Override
	public CompanyUser modify(CompanyUser newUser) {
		CompanyUser user = null;
		ResultSet set = getResultSet(MODIFY,
				newUser.getId(),newUser.getName(),
				newUser.getPassword(),newUser.getCompanyName(),
				newUser.getRealName(),newUser.getEmail(),
				newUser.getAddress(),newUser.getQQ(),
				newUser.getTel(),newUser.getIntroduce()
				);
		try {
			if (set != null && set.next()) {
				user = getUser(set);
